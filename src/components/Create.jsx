...
const Create = ({Product,setProduct}) => {
    ...
    return (
        <div className='w-100%'>
            <div className='lg:w-1/3 md:w-1/2 sm:w-1/1 absolute left-1/2 top-1/2 -translate-y-1/2 -translate-x-1/2  p-10 rounded-2xl' >
                <h1 className='text-3xl font-serif text-center'>Create Product</h1>
                <form onSubmit={submitHandler} className='flex justify-center flex-col gap-6 p-5  text-xl' >
                    <input
                    onChange={(e) => settitle(e.target.value)}
                    value={title} 
                    className='border-blue-200 border-2 rounded p-2 text-black' 
                    type='text' placeholder='title' ></input>

                    <textarea 
                    onChange={(e) => setdescription(e.target.value)}
                    value={description}
                    className='border-blue-200 border-2 rounded p-2' rows="2" 
                    placeholder='enter description'></textarea>

                    <input 
                    onChange={(e) => setimage(e.target.value)}
                    value={image}
                    className='border-blue-200 border-2 rounded p-2' 
